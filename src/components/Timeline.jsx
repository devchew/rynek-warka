...
export default function Timeline() {
  ...
  const timelineEvents = [
    ...
    },
    {
      ...
      description: 'Wprowadzenie produktów regionalnych i nawiązanie współpracy z lokalnymi artystami.'
    },
    {
      year: '2020',
      title: 'Nowe wyzwania',
      description: 'Dostosowanie działalności do nowych warunków i rozwinięcie usług na wynos.'
    },
    {
      year: '2022',
      title: 'Odnowienie wnętrza',
      description: 'Odświeżenie przestrzeni kawiarni i poszerzenie galerii o nowych twórców.'
    },
    {
      year: '2023',
      title: 'Rozwój oferty',
      description: 'Wprowadzenie nowych specjałów do menu i nawiązanie współpracy z lokalnymi dostawcami.'
    }
  ];

  return (
    <div className="py-12">
      <h2 className="font-serif text-3xl font-bold text-stone-800 mb-8 text-center">Nasza historia</h2>
          ...
          >
            <div className={`flex flex-col md:flex-row items-start ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              <div className="md:w-1/2 flex justify-center md:justify-start mb-4 md:mb-0">
                <div className={`flex items-center ${index % 2 === 0 ? 'md:pl-8' : 'md:pr-8 md:justify-end'}`}>
                  <div 
                    className={`flex items-center justify-center h-12 w-12 rounded-full shadow-md transition-all duration-300 z-10 ${
                      activeEvent === index ? 'bg-amber-600 text-white scale-110' : 'bg-white text-amber-600'
                    }`}
                  >
              ...
              <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:pr-8 md:text-right' : 'md:pl-8'}`}>
                ...
                >
                  <h3 className="font-serif text-xl font-bold text-amber-800 mb-2">{event.title}</h3>
