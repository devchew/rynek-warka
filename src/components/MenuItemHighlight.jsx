...
export default function MenuItemHighlight({ items }) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 my-8 shadow-md">
      <h3 className="font-serif text-xl text-center text-amber-800 mb-4">Polecane smaki</h3>
        ...
        {items.map((item, index) => (
          <div
            ...
            className={`p-4 rounded-lg cursor-pointer transition-all ${
              activeItem === index ? 'bg-amber-100 shadow-md' : 'bg-white hover:bg-amber-50'
            }`}
            onClick={() => setActiveItem(index)}
          >
            <div className="flex justify-between mb-1">
              <h4 className="font-medium text-stone-800">{item.name}</h4>
              <span className={`font-medium ${activeItem === index ? 'text-amber-700' : 'text-stone-600'}`}>
                {item.price}
              </span>
            </div>
            {activeItem === index && (
